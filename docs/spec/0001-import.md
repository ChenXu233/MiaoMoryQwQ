...
**目标**：用户选择一个文件夹后，应用扫描其中全部支持格式的照片，完成去重、解码、缩略图与入库，全程可见进度且可暂停/恢复，坏文件不阻塞批次。已导入文件夹的文件变更由 watcher 监听并自动增量同步（2026-09-09 所有者指示，自 LATER 提入本期）。

**非目标**：音视频导入（LATER）；S3 远程目录；导入时生成任何语义向量（P2）；删除文件的自动移除（同步只增不删，删除仍走手动入口）；多任务并行导入。
...
8. **增量预检**：scan 之后、整读之前，按（storage_key + size）与库内 ready 资产比对，未变化的文件直接跳过（不读盘不哈希）；因此「重导同一文件夹」与「watcher 增量同步」都只处理新增/大小变化的文件。判定为 MVP 语义：不比对 mtime，同大小改内容（罕见）需删除资产后重导。
9. **watcher 自动同步**：已注册且在线的文件夹由文件监听线程守护——变更去抖 ≥2s 后自动对该文件夹跑一次增量导入（复用同一引擎，单任务排队）；导入进行中发生的变更在任务结束后自动补扫一次；应用启动时对所有已注册文件夹做一次增量预检（只补新增，不重复处理）。
...
10. Given 已导入完成的文件夹新增 2 张照片，When 文件落盘稳定 ≥2s，Then 应用自动导入这 2 张（已有照片不重复处理、不重读），完成后自动补嵌并向量可搜。
11. Given 导入进行中向同一文件夹再复制 1 张照片，Then 本次任务结束后应用自动补扫并导入该照片。
12. Given 对已导入文件夹再次手动导入（或启动预检），Then 未变化的文件不产生读盘（增量预检跳过），仅新增/变化文件被处理。
