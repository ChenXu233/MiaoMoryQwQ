...
> 本文件沿用 `whitepaperv1.md` 的文件名以保留既有引用；内容版本为 v0.3，取代 v0.2（历史内容见 Git 记录与本文件附录 B 修订记录）。
...
| 架构决策记录 | `docs/decisions/` | ADR-0001 ~ 0010，历史不可改写 |
...
| 类别 | 内容 |
| :--- | :--- |
| **IN（MVP）** | 桌面端（Win/macOS/Linux）单二进制；照片导入（JPEG/PNG/WebP/HEIC，纯 Rust 解码，ADR-0008）；SHA-256 增量扫描与去重；缩略图与时间轴/网格浏览（虚拟滚动）+ Lightbox 预览；中文图文语义搜索；元数据过滤（日期/类型）；FTS5 文件名与文本检索；混合检索融合（RRF）；导入进度（可暂停/恢复/错误隔离）；模型下载（断点续传/离线提示）；自动更新器；键盘操作与基础无障碍 |
...
**时序调整（ADR-0010）**：一次性基准推迟至 P2 手工验收之后（验收不达标即提前恢复）；P2 先以临时选型 **Chinese-CLIP ViT-B/16**（MIT、官方 ONNX 导出路径、188M 参数 / 512 维）跑通语义检索，候选池与中文硬约束不变。
...
**数据布局：口袋式优先（v0.4 新增，ADR-0011）**：路径解析按四级优先单点集中于 `mm-platform`——① 环境变量 `MIAOMORY_DATA_DIR`（开发/测试）；② 显式便携（`<安装目录>\data\config.toml` 或 `.portable` 标记）；③ 自动便携（首次运行且安装目录可写）；④ 回退经典布局（「文档」工作区 + AppData 模型/配置/日志，用于 Program Files 等只读场景）。口袋模式下 `data\` 一个目录即全部用户数据（`index.db`、`thumbs\`、`models\`、`logs\`、`config.toml`），换机 = 拷贝目录。卸载/升级安全性已经 spike 核实（NSIS 仅删登记文件、MSI 仅删组件，非空 `data\` 均保留）。设置入口可查看/打开/更改数据位置（写配置重启生效）。

### 3.6 媒体解码：纯 Rust 栈（v0.3 修订，ADR-0008）

- **JPEG/PNG/WebP**：`image` crate 解码。
- **HEIC 解码**：iPhone 默认格式，由 `libheif-rs`（捆绑 libheif 原生库，Windows 经 vcpkg 提供）负责解码；EXIF 提取与方向校正用 `kamadak-exif`。
- **统一产出**：解码统一产出 RGB 缓冲，供缩略图生成与（P2 起）模型预处理共用。
- **复用**：FFmpeg 整体推迟到 LATER 音视频缩略图与转写阶段（届时另立 ADR）。
...
```
┌─────────────────────────────────────────────────────┐
│  UI (Vue 3 + TypeScript, 按 feature 组织)             │
├─────────────────────────────────────────────────────┤
...
```
选择文件夹 → Scan（枚举 + 忽略规则）
  → Hash（SHA-256 去重）→ Decode（image/libheif 解码 + EXIF 提取）
...
| 前端框架 | Vue 3 + TypeScript（ADR-0009） | 所有者裁定；性能与极简设计优先 |
| 媒体解码 | 纯 Rust：image + libheif-rs（ADR-0008） | 体积与许可证可控；FFmpeg 推迟 LATER |
| 验证策略 | TDD + 所有者手工验收；e2e 与模型基准延后（ADR-0010） | 先跑通核心闭环，质量责任分层 |
...
2. **CI 常驻内容**：格式/lint/clippy/tsc/单元测试/契约生成一致性/三平台构建矩阵；e2e 体验流程延后（ADR-0010），UI 与检索质量以项目所有者手工验证清单验收。
...
| HEIC 解码失败（个别文件） | 中 | 纯 Rust 解码（image + libheif-rs，ADR-0008）；失败文件进入 failed 队列并给出可读原因 |
...
v0.2 将白皮书从"可行性论证"升级为"产品规格 + 架构契约 + 交付基线"：MVP 收敛为**桌面 + 照片 + 中文语义搜索**（ADR-0003），向量检索交给 sqlite-vec（ADR-0002），中文模型选型以一次性基准落定（ADR-0006；基准推迟与临时选型见 ADR-0010），性能验收一次性人工执行、CI 只做确定性检查（ADR-0004；验证策略调整见 ADR-0010），交付按垂直切片推进（ADR-0007）。前端栈为 Vue 3 + TypeScript（ADR-0009），媒体解码为纯 Rust 栈（ADR-0008）。
...
### 前端
- Node.js 18+；Vue 3 + TypeScript + Vite；Tailwind CSS；依赖克制清单见 `AGENT.md`

### 媒体
- vcpkg（Windows 提供 libheif 原生库）；解码依赖 `image` / `libheif-rs` / `kamadak-exif`（ADR-0008）
...
| v0.3 | 2026-09-04 | MVP 启动决策落地：前端栈切换 Vue 3 + TypeScript（ADR-0009）；媒体解码改纯 Rust 栈（ADR-0008）；验证策略调整——TDD + 所有者手工验收、e2e 延后、模型一次性基准推迟并以 Chinese-CLIP ViT-B/16 临时选型（ADR-0010）；§4.7 决策表补齐前端框架/媒体解码/验证策略 |
| v0.4 | 2026-09-06 | 新增 §3.5 口袋式数据布局（ADR-0011：四级优先路径解析、`data\` 目录即全部用户数据、设置可迁移、卸载安全性 spike 通过）；清理 §2.2/§4.3/§8 残留的 FFmpeg 表述 |
