...
pub const MIGRATIONS: &[&str] = &[
    ...
    // v6：区域级语义索引（ADR-0015）——SAM 区域向量（vec0）、区域元数据、
    // 时空调制 DP-means 原型空间（章节 + 簇）、资产 GPS（时空调制空间轴，可空）。
    "ALTER TABLE assets ADD COLUMN gps_lat REAL;
    ALTER TABLE assets ADD COLUMN gps_lng REAL;
    CREATE VIRTUAL TABLE vec_regions USING vec0(
    ...
    CREATE TABLE regions (
        ...
        created_at INTEGER NOT NULL,
        UNIQUE(asset_id, region_idx)
    );
    CREATE INDEX idx_regions_asset ON regions(asset_id);
    CREATE INDEX idx_regions_cluster ON regions(cluster_id);
    CREATE TABLE region_chapters (
        chapter_id INTEGER PRIMARY KEY,
        started_at INTEGER NOT NULL,
        created_at INTEGER NOT NULL
    );
    INSERT INTO region_chapters (chapter_id, started_at, created_at) VALUES (0, 0, 0);
...
impl Store {
    ...
    pub fn list_region_clusters(&self) -> Result<Vec<(i64, i64, Vec<f32>, i64)>> {
        let rows = self
            .conn
            .prepare(
                "SELECT cluster_id, chapter_id, prototype, member_count
                 FROM region_clusters WHERE index_id = 2 ORDER BY cluster_id",
            )?
            .query_map([], |r| {
                let blob: Vec<u8> = r.get(2)?;
                let vec_f32: Vec<f32> = blob
                    .as_chunks::<4>()
                    .0
                    .iter()
                    .map(|c| f32::from_le_bytes(*c))
                    .collect();
                Ok((r.get(0)?, r.get(1)?, vec_f32, r.get(3)?))
            })?
            .collect::<std::result::Result<Vec<_>, _>>()?;
        Ok(rows)
    }
