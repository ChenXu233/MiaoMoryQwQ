...
//!
//! 分割：MobileSAM ONNX（encoder 整图一次 1024²；decoder 按批点提示，批内 4×4=16 点
//! 与 ONNX 固定批量对齐，16 批错位网格 → 全图 256 采样点）；输出低分辨率 mask（256²），
//! 在 256 域算面积/包围盒后映射回原图（省内存，无需全尺寸 mask）。
...
impl SamSegmenter {
    ...
    pub fn segment(
        ...
        for g in 0..GROUPS {
            ...
            let (_, sdata) = outs["scores"].try_extract_tensor::<f32>().map_err(|e| {
                tracing::error!(step = "decoder scores 提取", error = %e, "SAM ONNX 失败");
                ErrorCode::SearchUnavailable
            })?;
