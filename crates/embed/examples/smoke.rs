...
fn main() {
    ...
    let embedder = mm_embed::ClipEmbedder::load(dir, &manifest).expect("加载模型失败");
    ...
    let vecs = Embedder::embed_images(&embedder, &[img.clone(), img]).expect("图像嵌入失败");
    println!("image embedding dims: {}", vecs[0].len());

    let q = Embedder::embed_text(&embedder, "海边的日落").expect("文本嵌入失败");
